"""End-to-end test for the T1570 Lateral Tool Transfer detection.
...
from detlab.detector import detect_lateral_tool_transfer
...
def test_positive_fires(positive_records):
    alerts = detect_lateral_tool_transfer(positive_records)
    assert len(alerts) >= 1, "Expected at least one alert on positive fixture"
    a = alerts[0]
    assert a.orig_bytes >= 5_000_000


def test_negative_silent(negative_records):
    alerts = detect_lateral_tool_transfer(negative_records)
