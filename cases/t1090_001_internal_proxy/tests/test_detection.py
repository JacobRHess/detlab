"""End-to-end test for the T1090.001 Internal proxy / SOCKS chaining detection.
...
from detlab.detector import detect_internal_proxy
...
def test_positive_fires(positive_records):
    alerts = detect_internal_proxy(positive_records)
...
def test_negative_silent(negative_records):
    alerts = detect_internal_proxy(negative_records)
