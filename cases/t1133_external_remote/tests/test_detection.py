"""End-to-end test for the T1133 External Remote Services abuse detection.
...
from detlab.detector import detect_external_remote_services
...
def test_positive_fires(positive_records):
    alerts = detect_external_remote_services(positive_records)
    assert len(alerts) >= 1, "Expected at least one alert on positive fixture"
    a = alerts[0]
    assert a.distinct_destinations >= 1


def test_negative_silent(negative_records):
    alerts = detect_external_remote_services(negative_records)
