"""End-to-end test for the T1213.002 Information-Repository bulk read detection.
...
from detlab.detector import detect_info_repo_bulk_read
...
def test_positive_fires(positive_records):
    alerts = detect_info_repo_bulk_read(positive_records)
    assert len(alerts) >= 1, "Expected at least one alert on positive fixture"
    a = alerts[0]
    assert a.distinct_paths >= 100


def test_negative_silent(negative_records):
    alerts = detect_info_repo_bulk_read(negative_records)
